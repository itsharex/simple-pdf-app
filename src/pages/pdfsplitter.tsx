...
const PDFSplitter = () => {
    ...
    const handleSplitPageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (pdfFile && splitPages.length > 0) {
            try {
                ...
                reader.onload = async (e) => {
                    try {
                        const existingPdfBytes = new Uint8Array(e.target?.result as ArrayBuffer);
                        const pdfDoc = await PDFDocument.load(existingPdfBytes);
                        const splitDocs = await Promise.all(
                            splitPages.map(async (pageNumber) => {
                                const splitDoc = await PDFDocument.create();
                                if (pageNumber <= pdfDoc.getPageCount()) {
                                    const [copiedPage] = await splitDoc.copyPages(pdfDoc, [pageNumber - 1]);
                        ...
                        setErrorMessage('Failed to split the PDF. Please make sure the page numbers are correct.');
                        console.error(error);
                    }
                };
            ...
            setErrorMessage('Please select a PDF file and enter at least one page number to split.');
        }
    };
